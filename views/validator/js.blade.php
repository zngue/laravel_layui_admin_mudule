<script>
    var roleIndexUrl ="{{route('validator.index')}}",//列表数据
        roleChangeStatusUrl="{{route('validator.status')}}",
        roleDelUrl = "{{route('validator.del')}}",
        roleAddUrl = "{{route('validator.add')}}",
        roleSaveUrl = "{{route('validator.save')}}",
        doAddUrl = "{{route('validator.doAdd')}}",
        doSaveUrl="{{route('validator.doSave')}}",
        modeName = "表单后台验证规则"
</script>
    ...
    layui.use(['form','layer','laydate','table','laytpl'],function () {
        ...
        var roleTable = table.render({
            ...
            cols : [[
                ...
                {field: 'name', title: '名称', width:120},{field: 'status', title: '状态', width:100,templet:function (d) {
                        var checked='';
